...
def init_parser(parser):
    ...
    parser.add_argument("--validation-policy",
                        help="Path to validation policy file")
    parser.add_argument(
        "--skip-confirmation",
    ...
    parser.add_argument(
        "--policy-update-rate",
        help="Rate at which validation policy is updated (default: 0)",
        type=float,
        default=0,
    )
    parser.add_argument(
        "--client-wait-time",
        help="Time to wait before retrying client requests (default: {CLIENT_WAIT_TIME})",
        type=float,
        default=CLIENT_WAIT_TIME,
    )
...
class ScittUser(FastHttpUser):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.client = Client(self.host,
                             development=True,
                             wait_time=self.environment.parsed_options.client_wait_time)
...
class Submitter(ScittUser):
    ...
    def on_start(self):
        ...
        with open(self.environment.parsed_options.validation_policy,
                  "rb") as f:
            self.policy = f.read()

        self.policy_update_rate = self.environment.parsed_options.policy_update_rate

    @task(100)
    def select(self):
        if random.random() < self.policy_update_rate:
            self.update_validation_policy()
        else:
            self.submit_signed_statement()
    ...
    def update_validation_policy(self):
        self.trace(
            "policy_update", lambda: (self.client.post(
                "/policy",
