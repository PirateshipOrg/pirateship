...
def get_default_node_params(num_nodes, repeats, seconds):
    bench_params = {
        ...
        'nodes': 7,
        'workers': 1,
        'worker_fault_tolerance': 1,  # Number of workers each client sends to
        'rate': 10_000,
        'tx_size': 512,
        'duration': 60,
        'latency_warmup': 2,
...
ssh_cmd() {{
    ssh -o StrictHostKeyChecking=no -i {self.dev_ssh_key} "$@"
}}
scp_cmd() {{
    rsync -avz -e 'ssh -o StrictHostKeyChecking=no -i {self.dev_ssh_key}' "$@"
}}
                        ...
                        for i, addresses in enumerate(node_addrs):
                            for (id, addr) in addresses:
                                ...
                                cmd = CommandMaker.run_client(
                                    ...
                                    tx_size, num_clients,
                                    self.bench_params['worker_fault_tolerance'],
                                    threshold=1,
...
"""
            ...
            # Try again. Since metrics files somehow don't get copied the first time.
            for vm in set(list(self.binary_mapping.keys())):
                _script += f"""
scp_cmd {self.dev_ssh_user}@{vm.public_ip}:{self.remote_workdir}/logs/{repeat_num}/ {self.remote_workdir}/logs/{repeat_num}/ || true
"""
