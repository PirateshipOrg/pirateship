            ...
            on_crash_commit: Some(ProtoTransactionPhase {
                ops: vec![ProtoTransactionOp {
                    ...
                    operands: vec![
                        ...
                        format!("Tx:{}:{}", rand::random::<u64>(), rand::random::<u64>())
                            .into_bytes(),
                    ],
                    // operands: Vec::new(),
                }],
...
fn main() {
    ...
    for i in 0..SAMPLES {
        ...
        let block = ProtoBlock {
            ...
            view_is_stable: true,
            config_num: 0,
            sig: Some(pft::proto::consensus::proto_block::Sig::ProposerSig(
                vec![0u8; 64],
            )),
        };
    ...
    println!(
        "Mean large length: {} KiB Total: {} KiB",
        mean_large_len / 1024.0,
        mean_large_len * SAMPLES as f64 / 1024.0
    );

    // Calculate total size of all blocks
    let total_large_blocks_size: usize = large_blocks.iter().map(|b| b.encode_to_vec().len()).sum();
    println!(
        "Large blocks total size: {} KiB",
        total_large_blocks_size as f64 / 1024.0
    );
    ...
    println!(
        "Mean small length: {} KiB Total {} KiB",
        mean_small_len / 1024.0,
        mean_small_len * SAMPLES as f64 / 1024.0
    );

    // Calculate total size of all blocks
    let total_small_blocks_size: usize = small_blocks.iter().map(|b| b.encode_to_vec().len()).sum();
    println!(
        "Small blocks total size: {} KiB",
        total_small_blocks_size as f64 / 1024.0
    );
